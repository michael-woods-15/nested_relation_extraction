...
class OptunaHyperparameterSearch:
    def __init__(self, model_name, n_trials, seed=42):
        ...
        self.seed = seed
        self.metric_key = f"eval_{config_bert.METRIC_FOR_BEST_MODEL}"
        ...
        self.tokenizer, _ = load_tokenizer_and_classifier(self.model_name)

        label2id, id2label = get_labels()

        splits, split_paths = prepare_data_splits(
                config_bert.DATASET_PATH,
                config_bert.SPLITS_DIR,
                config_bert.SEED,
                config_bert.TRAIN_FRAC,
                config_bert.VAL_FRAC
            )
 
        self.tokenized_datasets = prepare_bert_dataset(
            split_paths,
            self.tokenizer,
            label2id,
            config_bert.MAX_LENGTH
        )
    ...
    def objective(self, trial):
        ...
        _, classifier = load_tokenizer_and_classifier(self.model_name)
        ...
        try:
            trainer = build_trainer(
                ...
                output_dir=config_bert.OUTPUT_DIR,
                ...
                logging_steps=config_bert.LOGGING_STEPS,
                metric_for_best_model=config_bert.METRIC_FOR_BEST_MODEL,
                greater_is_better=config_bert.GREATER_IS_BETTER,
